...
def create_booklet(numq, title_text, process_func, tex_template_file, tex_ans_template_file, output_filename_prefix, file_type="pdf"):
    ...
    timestamp = datetime.now(tz=pytz.timezone(
        "Australia/Melbourne")).strftime("%Y_%b_%d_%H_%M_%S")
    ...
    tex_template_txt = tex_template_txt.replace(
        "<<title>>", title_text).replace("<<diagrams>>", diagram_text)
    tex_template_txt_ans = tex_template_txt_ans.replace(
        "<<title>>", title_text).replace("<<diagrams>>", diagram_text_ans)
    ...
    merge_files(tex_output_path_pdf, tex_output_path_ans_pdf,
                tex_output_path_merged_pdf)

    if file_type == "zip":
        with zipfile.ZipFile(zip_output_path, "w") as zipf:
            zipf.write(tex_output_path_pdf, arcname=tex_output_path_pdf.name)
            zipf.write(tex_output_path_ans_pdf,
                       arcname=tex_output_path_ans_pdf.name)
            zipf.write(tex_output_path_merged_pdf,
                       arcname=tex_output_path_merged_pdf.name)
...
def get_keys(num):
    tex_keys_q_right = ["draw_style", "calc_sidelength1", "calc_sidelength2",
                        "sidelength1", "sidelength2", "rotation", "vA", "vB", "vC"]
    tex_keys_q_acute = ["draw_style", "calc_base", "calc_height",
                        "leftoffset", "base", "height", "rotation", "vA", "vB", "vC", "vD"]
    tex_keys_q_obtuse = ["draw_style", "calc_base", "calc_height",
                         "rightoffset", "base", "height", "rotation", "vA", "vB", "vC", "vD"]
...
def create_booklet_area_of_a_triangle(numq=20, triangle_type_num=4, title_text="Area of a Triangle", file_type="pdf", show_dimension_lines_bool=True, allow_rotation_bool=True, units="Random"):
    ...
    diagram_templates_dict = {i + 1: name for i,
                              name in enumerate(diagram_templates)}
    # Load diagram templates once and share with inner function
    currfile_dir = Path(__file__).parent
    diagram_template_paths_dict = {
        k: currfile_dir / v for k, v in diagram_templates_dict.items()}
    diagram_template_texts_dict = {
        k: path.read_text() for k, path in diagram_template_paths_dict.items()}

    def make_diagram_wrapper(idx):
        side_pair = side_pairs_list[idx - 1]
        rotation = rotations_list[idx - 1]
        tri_num = random.randint(
