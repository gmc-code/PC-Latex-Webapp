\documentclass[leqno, 12pt]{article}
\usepackage{tikz}
\usepackage{amsmath}
\usepackage{ulem}
\usetikzlibrary{angles,quotes,intersections,arrows.meta,calc}
\usepackage[a4paper, portrait, margin=1cm]{geometry}
\usepackage{multicol}
\usepackage{fancyhdr}

\def \HeadingAnswers {\section*{\Large Name: \underline{\hspace{8cm}} \hfill Date: \underline{\hspace{3cm}}} \vspace{-3mm}
{Measuring Angles: Answers} \vspace{1pt}\hrule}

% raise footer with page number; no header
\fancypagestyle{myfancypagestyle}{
  \fancyhf{}% clear all header and footer fields
  \renewcommand{\headrulewidth}{0pt} % no rule under header
  \fancyfoot[C] {\thepage} \setlength{\footskip}{14.5pt} % raise page number 6pt
}
\pagestyle{myfancypagestyle}  % apply myfancypagestyle

\begin{document}
\HeadingAnswers
\begin{multicols}{2}


\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 133^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=137]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (133:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=133, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {G};
      \node at ($(B)!-0.20cm!(midAC)$) {H};
      \node at ($(C)!-0.20cm!(B)$) {F};

      \ifdim 133 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 133 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 133 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 133^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 133^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 110^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=72]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (110:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=110, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {Y};
      \node at ($(B)!-0.20cm!(midAC)$) {X};
      \node at ($(C)!-0.20cm!(B)$) {Z};

      \ifdim 110 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 110 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 110 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 110^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 110^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 158^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=121]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (158:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=158, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {M};
      \node at ($(B)!-0.20cm!(midAC)$) {N};
      \node at ($(C)!-0.20cm!(B)$) {L};

      \ifdim 158 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 158 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 158 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 158^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 158^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill\columnbreak
    
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 130^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=286]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (130:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=130, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {Y};
      \node at ($(B)!-0.20cm!(midAC)$) {Z};
      \node at ($(C)!-0.20cm!(B)$) {X};

      \ifdim 130 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 130 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 130 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 130^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 130^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 172^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=7]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (172:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=172, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {T};
      \node at ($(B)!-0.20cm!(midAC)$) {S};
      \node at ($(C)!-0.20cm!(B)$) {R};

      \ifdim 172 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 172 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 172 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 172^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 172^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 133^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=96]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (133:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=133, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {H};
      \node at ($(B)!-0.20cm!(midAC)$) {F};
      \node at ($(C)!-0.20cm!(B)$) {G};

      \ifdim 133 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 133 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 133 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 133^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 133^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill\newpage
    

\end{multicols}
\end{document}

