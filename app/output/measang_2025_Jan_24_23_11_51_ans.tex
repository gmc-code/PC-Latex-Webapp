\documentclass[leqno, 12pt]{article}
\usepackage{tikz}
\usepackage{amsmath}
\usepackage{ulem}
\usetikzlibrary{angles,quotes,intersections,arrows.meta,calc}
\usepackage[a4paper, portrait, margin=1cm]{geometry}
\usepackage{multicol}
\usepackage{fancyhdr}

\def \HeadingAnswers {\section*{\Large Name: \underline{\hspace{8cm}} \hfill Date: \underline{\hspace{3cm}}} \vspace{-3mm}
{Measuring Angles: Answers} \vspace{1pt}\hrule}

% raise footer with page number; no header
\fancypagestyle{myfancypagestyle}{
  \fancyhf{}% clear all header and footer fields
  \renewcommand{\headrulewidth}{0pt} % no rule under header
  \fancyfoot[C] {\thepage} \setlength{\footskip}{14.5pt} % raise page number 6pt
}
\pagestyle{myfancypagestyle}  % apply myfancypagestyle

\begin{document}
\HeadingAnswers
\begin{multicols}{2}


\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 16^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=44]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (16:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=16, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {B};
      \node at ($(B)!-0.20cm!(midAC)$) {C};
      \node at ($(C)!-0.20cm!(B)$) {A};

      \ifdim 16 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 16 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 16 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 16^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 16^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 21^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=15]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (21:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=21, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {B};
      \node at ($(B)!-0.20cm!(midAC)$) {A};
      \node at ($(C)!-0.20cm!(B)$) {C};

      \ifdim 21 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 21 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 21 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 21^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 21^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 53^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=32]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (53:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=53, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {X};
      \node at ($(B)!-0.20cm!(midAC)$) {Y};
      \node at ($(C)!-0.20cm!(B)$) {Z};

      \ifdim 53 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 53 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 53 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 53^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 53^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill\columnbreak
    
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 185^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=30]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (185:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=185, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {A};
      \node at ($(B)!-0.20cm!(midAC)$) {C};
      \node at ($(C)!-0.20cm!(B)$) {B};

      \ifdim 185 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 185 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 185 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 185^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 185^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 130^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=58]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (130:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=130, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {X};
      \node at ($(B)!-0.20cm!(midAC)$) {Z};
      \node at ($(C)!-0.20cm!(B)$) {Y};

      \ifdim 130 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 130 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 130 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 130^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 130^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill
\begin{equation}
  \hspace{0.5cm} \parbox[t]{\textwidth}{%
    \hspace{2.5cm} $\theta^\circ = 105^\circ$ \\
  \begin{tikzpicture}[scale=1.0, baseline={([yshift=-1pt]current bounding box.north)}]
    \begin{scope}[rotate=56]
      \coordinate (B) at (0,0);
      \coordinate (C) at (4,0);
      \coordinate (A) at (105:4cm);
      \coordinate (midAC) at ($(A)!0.5!(C)$);
      \coordinate (Q) at ($(B)!0.5!(C) + (0.5cm,-0.55cm)$);


      % Draw the angle
      \draw (B) -- (C);
      \draw (B) -- (A);

      % Mark angles with arcs;
      \draw ($(B)!0.5cm!(C)$) arc [start angle=0, end angle=105, radius=0.5cm];

      % Label angles
      \node at ($(A)!-0.20cm!(B)$) {T};
      \node at ($(B)!-0.20cm!(midAC)$) {R};
      \node at ($(C)!-0.20cm!(B)$) {S};

      \ifdim 105 pt < 15pt
          % Case: Angle is less than 40°
          \node at ($(B) + (1.7cm,0.15cm)$) {$\theta^\circ$};
      \else
        \ifdim 105 pt < 40pt
            % Case: Angle is less than 40°
            \node at ($(B) + (1.1cm,0.15cm)$) {$\theta^\circ$};
        \else
          \ifdim 105 pt > 180pt
              % Case: Angle is greater than 180°
              \node at ($(B)!-0.80cm!(midAC)$) {$\theta^\circ$};
          \else
              % Case: Angle is between 40° and 180°
              \node at ($(B)!0.75cm!(midAC)$) {$\theta^\circ$};
          \fi
        \fi
      \fi

      % Mark angles in degrees
      % \node at ($(B)!2.0cm!(midAC)!0.5cm!(C)$) {$\theta^\circ = 105^\circ$};
      % \node at ($(Q)$) {$\theta^\circ = 105^\circ$};

    \end{scope}
  \end{tikzpicture}
  }
\end{equation}\vspace{1cm} \vfill\newpage
    

\end{multicols}
\end{document}

